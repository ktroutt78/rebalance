...
- **Mixed fleet (added 2026-07):** three vehicle types — bike trailer (3), cargo van (12), box truck (30) — with per-vehicle capacities in the solver (capacity-to-cluster matching + oversized-station eviction; distance objective unchanged). Dollar costs live OUTSIDE the solver, in `config.js`: fixed dispatch + per-mile + **overtime** (time-and-a-half of plausible NYC crew wages) for hours past the 8-hour overnight shift (drive time at city speeds + per-stop/per-bike service time). Overtime is a soft penalty, not a feasibility wall — a lone box truck's 23-hour route is allowed but carries ~15 h of overtime and prices itself out. The fleet finder recommends the cheapest full-coverage mix with overtime included. Stations are served atomically — no split deliveries.
